...
public class Day6 {
  ...
  public static void main(String[] args) {
    List<Integer> sampleIntegers = Arrays.stream(Utils.readFileAsString(SAMPLE_INPUT).split(","))
        .map(Integer::parseInt)
        .collect(Collectors.toList());

    List<Integer> integers = Arrays.stream(Utils.readFileAsString(INPUT).split(","))
        .map(Integer::parseInt)
        .collect(Collectors.toList());
  ...
  private static void process(List<Integer> integers, int daysToSimulate) {
   ...
   while (days <= daysToSimulate) {
    ...
    Utils.log("Fish after %d days: %d", days, countOfFish);
  }
}
